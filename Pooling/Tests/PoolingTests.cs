...
namespace FLZ.Pooling
{
    ...
    public class PoolingTests<T> where T : class, IPoolable, new()
    {
        private MockPool<T> mockPool;
        ...
        {
            mockPool = new MockPool<T>(size, factory);
        ...
        {
            mockPool = new MockPool<T>(size, factory);
            MockPooledObject spawnedObject = mockPool.Spawn() as MockPooledObject; 
            
            mockPool.DeSpawn(spawnedObject as T);

            Assert.IsFalse(spawnedObject.Spawned);
        }
    }

    public class MockPool<T> : AbstractPool<T> where T : class, IPoolable
    {
